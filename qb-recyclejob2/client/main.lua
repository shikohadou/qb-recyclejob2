...
CreateThread(function()
    ...
    while true do
        ...
        if #(pos - vector3(1088.5, -3101.31, -39.0)) < 1 and not IsPedInAnyVehicle(PlayerPedId(), false) and carryPackage == nil then
            DrawMarker(2, 1088.5, -3101.31, -38.0, 0.9, 0, 0, 0, 0, 0, 0.2001, 0.2001, 0.2001, 255, 255, 255, 255, 0, 0, 0, 0)
            if #(pos - vector3(1088.5, -3101.31, -38.0)) < 1.3 then
                if onDuty then
                    DrawText3D(1088.5, -3101.31, -39.0, "~g~E~w~ - Clock Out")
                else
                    DrawText3D(1088.5, -3101.31, -39.0, "~g~E~w~ -  Clock In")
                end
                if IsControlJustReleased(0, 38) then
                    onDuty = not onDuty
                    if onDuty then
            ...
            if packagePos ~= nil then
                ...
                if carryPackage == nil then
                    if #(pos - vector3(packagePos.x, packagePos.y, packagePos.z)) then
                        DrawText3D(packagePos.x,packagePos.y,packagePos.z+ -2, "~g~E~w~ - Pack Package")
                        if IsControlJustReleased(0, 38) then
